...
switch (command) {
case 'menu': case 'help': case 'octopus':
...
case 'infobot':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			reply('Bot WhatsApp fast respon. run with Heroku')
			break
//tesbot
case 'tes': case 'test':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			break
//toxic
case 'kontol': case 'jancok': case 'cok':
if (!isHaruka) return sendButLocation(from, lang.dosa(pushname), '© ' + ownername, thumbnail, [{buttonId: '.astagfirullah', buttonText:{displayText: 'Astagrirullah'}, type: 1}], {quoted: mek})
				break
case 'owner':{
	if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			...
			ini_list.push({
				...
				hehe = await haruka.sendMessage(from, {
					...
					haruka.sendMessage(from, `Nih Kak Contact Owner Ku, Sv Ya 🤝`, text, {quoted: hehe})
				}
			break
